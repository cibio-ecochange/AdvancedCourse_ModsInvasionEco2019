...
a mountainous area located in Peneda-Ger?s National Park, NW Portugal. Total precipitation 
amounts to 1300 mm/year and mean annual temperature to 13 ?C, with large amplitude in mean 
annual values between cold and warm seasons (-10 to 35 ?C). Elevation ranges from 145 to 1253m 
with steep slopes.                  

Scrub and deciduous oak forests developed on granite bedrock and are the main vegetation types, 
as a follow-up of the abandonment of extensive husbandry. Pine forests are also another relevant 
vegetation type, especially on common land. Agricultural areas occupy a small and decreasing 
extent at lower altitudes.              

_A. dealbata_ was locally introduced by forestry services to stabilize slopes around the start 
of the XX^th^ century. However, due to management practices, abandonment and wildfires in the 
region, it has become widespread throughout the landscape.                 
...
![Exercise workflow](./_img/COST_Action_WorkFlow_RS_OBIA_exercise-v1.png)


To map the target species we will use _SegOptim_ - a R package developed in PRECOL team 
(still in beta!...) used to perform object-based analyses of EO data. _SegOptim_ allows 
to interface multiple GIS/RS analysis software, such as Orfeo Toolbox (OTB), GRASS, SAGA, 
etc. and use them to perform image segmentation. Using the segmented image, we can then run 
supervised classification, which allows combining field or digitized training data with spectral 
image features to obtain maps with the distribution of target species or land cover types. 
...
training a classifier. The input format can be of any type included in the `raster` 
package via `rgdal` ([link](http://www.gdal.org/formats_list.html)).
...
for classification (e.g., spectral bands and indices, texture, elevation, slope). The input 
format can be of any type included in the `raster` package via `rgdal` 
([link](http://www.gdal.org/formats_list.html)). This will allow the classification algorithm 
to extract/learn the spectral profile and get the classification rules.
...
Before beginning, start by copying the dropbox folder in __NNEXT_TS_materials/Practicals materials/Remote_Sensing_Exercises__ to your local file system. Otherwise, changes in these files 
will be passed to other trainees :-o
...
Change the paths in the following code section according to your system but keeping the 
terminations/file name. These are are inputs and outputs

For example:        

"D:/MyDocs/temp/TrainingSchool/SEGM_FEAT/WV2_b532_VilarVeiga_smallTestSite.tif"      

to -->      

"/Users/myName/NN_COST_TS/SEGM_FEAT/WV2_b532_VilarVeiga_smallTestSite.tif"        
          
Then, run the code chunk by clicking the play button (green triangle). This will store file paths 
in R memory.
...
inputSegFeat.path <- "D:/MyDocs/temp/TrainingSchool/SEGM_FEAT/WV2_b532_VilarVeiga_smallTestSite.tif"

# Path to training raster data
# [0] Non-invaded areas [1] Acacia dealbata invaded areas
# In TRAIN_AREAS directory
trainData.path <- "D:/MyDocs/temp/TrainingSchool/TRAIN_AREAS/trainAreas_Adealbata_VVeiga_WV2_v1.tif"

# Path to raster data used as classification features
# In CLASSIF_FEAT directory
classificationFeatures.path <- 
  c("D:/MyDocs/temp/TrainingSchool/CLASSIF_FEAT/WV2_NDIcombns_VilarVeiga_smallTestSite.tif",
    "D:/MyDocs/temp/TrainingSchool/CLASSIF_FEAT/WV2_VilarVeiga_smallTestSite.tif")
...
If you receive a message from the above code chunk you have to correct your data paths.
...
Also change these paths according to your file system and working directory. In this case you can change
the output file names. Click play. 

```{r Set raster outputs}    

## Output file from OTB image segmentation     
outSegmRst.path <- "D:/MyDocs/temp/TrainingSchool/segmRaster.tif"

# The final output file containing the distribution of the target species
outClassRst.path <- "D:/MyDocs/temp/TrainingSchool/WV2_VilarVeiga_AcaciaSpp_v1.tif"
...
Mean shift algorithm was firstly proposed by Fukunaga and Hostetler (1975) as a non-parametric 
density estimation algorithm that converges rapidly on the local maximum of a probability 
density function through iterations. It was successfully applied to image segmentation by 
Comaniciu and Meer (1999), with the advantage of not requiring prior knowledge of the number 
of clusters and not constraining the shape of the clusters. The implementation in OTB, extends 
the mean-shift algorithm so it can handle large images. Go to: [link](https://goo.gl/DHp5rh) for 
more details on mean-shift or read the paper on LSMS:      
         
Michel, J. et al. (2015). _Stable Mean-Shift Algorithm and Its Application to the Segmentation of 
Arbitrarily Large Remote Sensing Images_. IEEE Transactions on Geoscience and Remote Sensing 
(53-2:952-964).       
...
__NOTE__:    
Image segmentation parameters need proper tuning to obtain the best results out of object-based 
analyses. This requires testing different parametrizations and values to find which values 
fit the best to each case. _SegOptim_ (as the name suggests) allows to optimize these parameters! 
However, that subject would require another session... ;-) 
...
spectral bands) to map the distribution of _A. dealbata_. The Random Forest (RF) 
algorithm will be used for this purpose. RF are an ensemble learning method for 
...
90% train / 10% test). Cohen's _Kappa_ was selected as the evaluation metric (although others 
can be calculated later). A minimum of 30 total train cases were set, and in each train/test 
